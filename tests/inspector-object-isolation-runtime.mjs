...
try {
  ...
  const objectMutationEvidence = process.env.PHASE037A_DIAGNOSTIC === '1' ? null : await evaluate(`(async()=>{
    ...
    const fingerprint=node=>{const rect=node.getBoundingClientRect();const style=getComputedStyle(node);return {inline:node.getAttribute('style')||'',rect:[rect.x,rect.y,rect.width,rect.height].map(value=>+value.toFixed(2)),style:[style.transform,style.translate,style.rotate,style.width,style.height,style.opacity,style.border,style.borderRadius,style.color,style.textAlign,style.filter,style.boxShadow,style.outline]}};
  ...
  if (dependencyAlignmentEvidence) {
    ...
    assert(dependencyAlignmentEvidence.thicknessOff && dependencyAlignmentEvidence.thicknessOn && /url\(/.test(dependencyAlignmentEvidence.imageFilter) && dependencyAlignmentEvidence.designBorderWidth === '0px' && dependencyAlignmentEvidence.alphaRadius === '5', `Outline dependency is not functional: ${JSON.stringify(dependencyAlignmentEvidence)}`)
