...
try {
  ...
  const configured = await evaluate(`(async()=>{
    const tick=()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));const settle=async()=>{await new Promise(resolve=>setTimeout(resolve,240));await tick()};const editor=document.querySelector('#app').__vue_app__.config.globalProperties.$pinia._s.get('editor');const open=async(category)=>{const group=document.querySelector('[data-property-category="'+category+'"]');const toggle=group?.querySelector('.accordion-toggle');if(!group||!toggle)throw new Error('Accordion not found for '+category);if(toggle.getAttribute('aria-expanded')!=='true'){toggle.click();await settle()}else await tick();return group};const group=await open('animation');
    ...
    const durationBefore=!document.querySelector('[data-property-key="animation.duration"]');set('animation.type','fade');await tick();const durationAfter=document.querySelector('[data-property-key="animation.duration"] input')?.disabled??true;set('animation.duration',450);set('animation.delay',100);set('animation.ease','ease-out');set('animation.playOnce',false);await tick();set('animation.loop',true);await tick();set('animation.direction','alternate');await open('interaction');set('animation.hover','scale');set('animation.click','ripple');await open('animation');set('animation.scroll','reveal');await tick();set('animation.scrollPlayback','replay');set('animation.scrollOffset',120);set('animation.scrollThreshold',.25);set('animation.text','typewriter');set('animation.timelineMode','sequential');set('animation.timelineDelay',80);await tick();await open('advanced');set('animation.fillMode','forwards');const fillModeAdvanced=Boolean(document.querySelector('[data-inspector-key="advanced:animation.fillMode"]'));await open('animation');await tick();
    const animation=await import('/src/editor/animationRegistry.ts');const record=editor.draftSnapshot.animations['portfolio-hero'];const config=animation.decodeAnimationConfiguration(record.name);const fields=[...group.querySelectorAll('[data-property-key]')].map(item=>item.dataset.propertyKey);return {selected:editor.selectedObjectId,durationBefore,durationAfter,record:{...record},config,history:editor.commandHistory.length,dirty:editor.hasUnsavedChanges,active:editor.activeAccordion,fields,fillModeAdvanced,encodedLength:record.name.length,loopDisabled:document.querySelector('[data-property-key="animation.loop"] select,input,button')?.disabled??true,directionDisabled:document.querySelector('[data-property-key="animation.direction"] select,input,button')?.disabled??true};
  })()`)
  ...
  const globalDisableEvidence = await evaluate(`(async()=>{const tick=()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));const waitForReason=async(reason)=>{const started=performance.now();while(document.querySelector('.preview-stage')?.dataset.animationsDisabled!==reason&&performance.now()-started<1200)await new Promise(resolve=>setTimeout(resolve,20))};const group=document.querySelector('[data-property-category="animation"]');if(group.querySelector('.accordion-toggle').getAttribute('aria-expanded')!=='true'){group.querySelector('.accordion-toggle').click();await new Promise(resolve=>setTimeout(resolve,240));await tick()}const toggle=document.querySelector('[data-property-key="animation.globalDisabled"] input');toggle.checked=true;toggle.dispatchEvent(new Event('change',{bubbles:true}));await waitForReason('global');await tick();const root=document.querySelector('.preview-stage');const previewHidden=!document.querySelector('[data-property-key="animation.preview"]');const global=document.querySelector('#app').__vue_app__.config.globalProperties.$pinia._s.get('editor').draftSnapshot.animations.__global__.enabled;const rootReason=root.dataset.animationsDisabled;toggle.checked=false;toggle.dispatchEvent(new Event('change',{bubbles:true}));await waitForReason('false');await tick();const previewReEnabled=!document.querySelector('[data-property-key="animation.preview"] button')?.disabled;return {previewHidden,previewReEnabled,global,rootReason,reEnabled:document.querySelector('#app').__vue_app__.config.globalProperties.$pinia._s.get('editor').draftSnapshot.animations.__global__.enabled,reEnabledReason:root.dataset.animationsDisabled};})()`)
  assert(globalDisableEvidence.previewHidden && globalDisableEvidence.previewReEnabled && globalDisableEvidence.global === false && globalDisableEvidence.rootReason === 'global' && globalDisableEvidence.reEnabled === true && globalDisableEvidence.reEnabledReason === 'false', `global animation disable failed: ${JSON.stringify(globalDisableEvidence)}`)
