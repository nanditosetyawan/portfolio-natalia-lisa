...
try {
  ...
  const typographyEvidence = await evaluate(`(async()=>{
    ...
    const typography=document.querySelector('[data-property-category="font"]');if(typography.querySelector('.accordion-toggle')?.getAttribute('aria-expanded')!=='true')typography.querySelector('.accordion-toggle').click();await tick();const editor=document.querySelector('#app').__vue_app__.config.globalProperties.$pinia._s.get('editor');const design=document.querySelector('#app').__vue_app__.config.globalProperties.$pinia._s.get('designSystem');const field=document.querySelector('[data-inspector-key="simple:font.size"]');const input=field.querySelector('input');input.value='48';input.dispatchEvent(new Event('input',{bubbles:true}));await tick();const overridden=field.closest('.property-field').querySelector('.friendly-style-state span')?.textContent.trim();field.closest('.property-field').querySelector('.friendly-style-state button')?.click();await tick();const reset=editor.draftSnapshot.typography['portfolio-hero']?.fontSize;editor.undo();await tick();const undo=editor.draftSnapshot.typography['portfolio-hero']?.fontSize;editor.redo();await tick();const redo=editor.draftSnapshot.typography['portfolio-hero']?.fontSize;
  ...
  assert(typographyEvidence.assignment.roleId === 'heading-1' && typographyEvidence.fontFamily.includes('Georgia') && typographyEvidence.fontWeight === 700 && typographyEvidence.overridden === 'Custom style' && typographyEvidence.reset === '4rem' && typographyEvidence.undo === '48px' && typographyEvidence.redo === '4rem' && typographyEvidence.history <= 10 && typographyEvidence.selected === 'portfolio-hero', `global typography inheritance/reset failed: ${JSON.stringify(typographyEvidence)}`)
