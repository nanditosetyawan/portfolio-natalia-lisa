...
try {
  ...
  assert(!panelSemantics.reuseVisible && !panelSemantics.genericShadowVisible && panelSemantics.imageShadowCategory === 'media' && !panelSemantics.shadowLabels.includes('Spread'), `Media action/effect deduplication failed: ${JSON.stringify(panelSemantics)}`)
  ...
  const actionSetup = await evaluate(`(async()=>{const pinia=document.querySelector('#app').__vue_app__.config.globalProperties.$pinia;const editor=pinia._s.get('editor');const library=pinia._s.get('media-library');await library.refresh();window.__phase037cOpened=[];window.open=(href)=>{window.__phase037cOpened.push(String(href));return null};const assignment=editor.draftSnapshot.media.assignments.find(item=>item.entityId==='portfolio-profile-media');return {assignment:assignment?.assetId,assets:library.assets.length,assetIds:library.assets.map(item=>item.id),history:editor.commandHistory.length,selected:editor.selectedObjectId}})()`)
  const uploadFile = path.join(profilePath, 'phase037c-new-image.svg')
  await writeFile(uploadFile, '<svg xmlns="http://www.w3.org/2000/svg" width="80" height="120"><path fill="#b85b69" d="M40 0 80 120H0z"/></svg>')
  ...
  const upload = await evaluate(`(()=>{const pinia=document.querySelector('#app').__vue_app__.config.globalProperties.$pinia;const editor=pinia._s.get('editor');const library=pinia._s.get('media-library');const previousIds=new Set(${JSON.stringify(actionSetup.assetIds)});const asset=library.assets.find(item=>!previousIds.has(item.id));return {asset:asset?{id:asset.id,name:asset.name,path:asset.storagePath,persisted:asset.metadataPersisted}:null,assetNames:library.assets.map(item=>({id:item.id,name:item.name,path:item.storagePath})),assignment:editor.draftSnapshot.media.assignments.find(item=>item.entityId==='portfolio-profile-media')?.assetId,assets:library.assets.length,libraryError:library.error,status:document.querySelector('.save-status')?.textContent?.trim()??'',history:editor.commandHistory.length,selected:editor.selectedObjectId}})()`)
  assert(upload.asset, `Upload New Image did not register an Asset Library row: ${JSON.stringify(upload)}`)
  assert(upload.assignment === actionSetup.assignment && upload.assets === actionSetup.assets + 1 && upload.history === actionSetup.history && upload.asset.path.startsWith('draft/library/') && upload.asset.persisted && upload.selected === actionSetup.selected, `Upload New Image replaced the page instance or bypassed Asset Library: ${JSON.stringify({actionSetup,upload})}`)
  ...
  const browse = await evaluate(`(async()=>{const tick=()=>new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));const pinia=document.querySelector('#app').__vue_app__.config.globalProperties.$pinia;const editor=pinia._s.get('editor');const before=editor.draftSnapshot.media.assignments.find(item=>item.entityId==='portfolio-profile-media')?.assetId;const history=editor.commandHistory.length;const card=document.querySelector('[data-asset-card-id="'+CSS.escape(${JSON.stringify(upload.asset.id)})+'"]');card.dispatchEvent(new MouseEvent('dblclick',{bubbles:true,cancelable:true}));await tick();return {before,after:editor.draftSnapshot.media.assignments.find(item=>item.entityId==='portfolio-profile-media')?.assetId,historyBefore:history,historyAfter:editor.commandHistory.length,pickerClosed:!document.querySelector('.asset-picker'),opened:window.__phase037cOpened,title:document.querySelector('.asset-picker h2')?.textContent??'closed'};})()`)
  assert(browse.before === browse.after && browse.historyBefore === browse.historyAfter && browse.pickerClosed && browse.opened.some(url=>url.includes(encodeURIComponent(upload.asset.id))), `Choose from Media silently replaced the selected image: ${JSON.stringify(browse)}`)

  const replaceAssetsBefore = upload.assets
  const replaceFile = path.join(profilePath, 'phase037c-replacement.svg')
  await writeFile(replaceFile, '<svg xmlns="http://www.w3.org/2000/svg" width="90" height="110"><circle cx="45" cy="55" r="42" fill="#49362f"/></svg>')
  await setFileInput('[data-property-key="media.replace"] input[type="file"]', replaceFile)
  await waitFor(`(()=>{const pinia=document.querySelector('#app').__vue_app__.config.globalProperties.$pinia;const editor=pinia._s.get('editor');const library=pinia._s.get('media-library');return !library.mutating&&library.assets.some(asset=>asset.name==='phase037c-replacement')&&editor.draftSnapshot.media.assignments.find(item=>item.entityId==='portfolio-profile-media')?.assetId===library.assets.find(asset=>asset.name==='phase037c-replacement')?.id})()`)
  const replace = await evaluate(`(()=>{const pinia=document.querySelector('#app').__vue_app__.config.globalProperties.$pinia;const editor=pinia._s.get('editor');const library=pinia._s.get('media-library');const asset=library.assets.find(item=>item.name==='phase037c-replacement');return {newId:asset.id,path:asset.storagePath,assignment:editor.draftSnapshot.media.assignments.find(item=>item.entityId==='portfolio-profile-media')?.assetId,assets:library.assets.length,oldAssetStillPresent:library.assets.some(item=>item.id===${JSON.stringify(actionSetup.assignment)}),oldReferenceStillPresent:editor.draftSnapshot.media.references.some(item=>item.assetId===${JSON.stringify(actionSetup.assignment)}),command:editor.commandHistory.at(-1)?.type,selected:editor.selectedObjectId}})()`)
  assert(replace.assignment === replace.newId && replace.assets === replaceAssetsBefore + 1 && replace.oldAssetStillPresent && replace.oldReferenceStillPresent && replace.command === 'REPLACE_MEDIA' && replace.selected === 'portfolio-profile-media', `Replace Selected Image was unsafe or mistargeted: ${JSON.stringify(replace)}`)
