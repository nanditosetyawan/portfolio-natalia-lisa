...
try {
  ...
  const bindingEvidence = await evaluate(`(async()=>{
    ...
    document.querySelector('[data-property-key="media.remove"]').click();await tick();const removed=!editor.draftSnapshot.media.assignments.some(item=>item.entityId===area);const orphanReferenceRemoved=!editor.draftSnapshot.media.references.some(item=>item.assetId===applied);editor.undo();await tick();const restored=editor.draftSnapshot.media.assignments.find(item=>item.entityId===area)?.assetId===applied;
