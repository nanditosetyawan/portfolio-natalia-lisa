...
try {
  ...
  const restoredCrudEvidence = await evaluate(`(async()=>{
    ...
    const rules=await import('/src/lib/mediaUploadRules.ts');const input=document.querySelector('.upload-workspace input[type="file"]');const accept=input.accept;const beforeInvalid=library.assets.length;const invalidTransfer=new DataTransfer();invalidTransfer.items.add(new File(['invalid'],'phase33a-rejected.png',{type:'image/png'}));input.files=invalidTransfer.files;input.dispatchEvent(new Event('change',{bubbles:true}));await tick();const invalidError=document.querySelector('.error-banner')?.textContent?.trim()??'';const invalidRejected=library.assets.length===beforeInvalid&&document.querySelector('.queue-count')?.textContent?.includes('0/5');const transfer=new DataTransfer();transfer.items.add(new File([new TextEncoder().encode('%PDF-1.4\\n%%EOF')],'phase33a-proof.pdf',{type:'application/pdf'}));input.files=transfer.files;input.dispatchEvent(new Event('change',{bubbles:true}));await tick();document.querySelector('.submit-upload-btn').click();
    ...
    return {uploaded:Boolean(uploaded),storagePath:uploaded?.storagePath,renamed,realPreview,deleteEnabled,deleted,accept,expectedAccept:rules.MEDIA_LIBRARY_FILE_ACCEPT,invalidRejected,invalidError};
  })()`)
  assert(restoredCrudEvidence.uploaded && restoredCrudEvidence.storagePath?.startsWith('draft/library/') && restoredCrudEvidence.renamed && restoredCrudEvidence.realPreview && restoredCrudEvidence.deleteEnabled && restoredCrudEvidence.deleted && restoredCrudEvidence.accept === restoredCrudEvidence.expectedAccept && restoredCrudEvidence.invalidRejected && /\.png|WEBP|PDF|GIF/i.test(restoredCrudEvidence.invalidError), `restored Upload/Document repository workflow failed: ${JSON.stringify(restoredCrudEvidence)}`)
