...
try {
  ...
  for (const label of ['Upload','Choose from Media','Replace','Remove','Reuse this image','Show in Media Library']) assert(editorEvidence.controls.includes(label), `Editor media action missing: ${label}`)
  ...
  const bindingEvidence = await evaluate(`(async()=>{
    ...
    const beforeRefs=editor.draftSnapshot.media.references.length;document.querySelector('[data-property-key="media.duplicateReference"] button').click();await tick();const duplicated=editor.draftSnapshot.media.assignments.find(item=>item.entityId===area)?.assetId;const afterRefs=editor.draftSnapshot.media.references.length;editor.undo();await tick();
    document.querySelector('[data-property-key="media.remove"] button').click();await tick();const removed=!editor.draftSnapshot.media.assignments.some(item=>item.entityId===area);const orphanReferenceRemoved=!editor.draftSnapshot.media.references.some(item=>item.assetId===applied);editor.undo();await tick();const restored=editor.draftSnapshot.media.assignments.find(item=>item.entityId===area)?.assetId===applied;
    let revealed='';const originalOpen=window.open;window.open=(url)=>{revealed=String(url);return null};document.querySelector('[data-property-key="media.reveal"] button').click();window.open=originalOpen;
    ...
    return {applied,command,pickerClosed,beforeRefs,afterRefs,duplicated,removed,orphanReferenceRemoved,restored,revealed,highlighted,dropped,previewSource,selection:editor.selectedObjectId,history:editor.commandHistory.length};
  })()`)
  ...
  assert(bindingEvidence.duplicated !== bindingEvidence.applied && bindingEvidence.afterRefs === bindingEvidence.beforeRefs + 1 && bindingEvidence.removed && bindingEvidence.orphanReferenceRemoved && bindingEvidence.restored, 'duplicate/remove/undo reference workflow failed')
