...
try {
  ...
  for (const label of ['Upload New Image','Choose from Media','Replace Selected Image','Remove Selected Image','Show in Media Library']) assert(editorEvidence.controls.includes(label), `Editor media action missing: ${label}`)
  ...
  const bindingEvidence = await evaluate(`(async()=>{
    ...
    const area='portfolio-profile-media';const opened=[];const originalOpen=window.open;window.open=(url)=>{opened.push(String(url));return null};const before=editor.draftSnapshot.media.assignments.find(item=>item.entityId===area)?.assetId;const historyBefore=editor.commandHistory.length;
    const pickerCard=document.querySelector('[data-asset-card-id="'+CSS.escape(asset.id)+'"]');pickerCard.dispatchEvent(new MouseEvent('dblclick',{bubbles:true,cancelable:true}));await tick();const afterBrowse=editor.draftSnapshot.media.assignments.find(item=>item.entityId===area)?.assetId;const historyAfterBrowse=editor.commandHistory.length;const pickerClosed=!document.querySelector('.asset-picker');const duplicateHidden=!document.querySelector('[data-property-key="media.duplicateReference"]');
    document.querySelector('[data-property-key="media.remove"] button').click();await tick();const removed=!editor.draftSnapshot.media.assignments.some(item=>item.entityId===area);const underlyingAssetRetained=library.assets.some(item=>item.id===before);const orphanReferenceRemoved=!editor.draftSnapshot.media.references.some(item=>item.assetId===before);editor.undo();await tick();const restored=editor.draftSnapshot.media.assignments.find(item=>item.entityId===area)?.assetId===before;
    document.querySelector('[data-property-key="media.reveal"] button').click();
    ...
    const command=editor.commandHistory.at(-1)?.type;window.open=originalOpen;return {before,afterBrowse,historyBefore,historyAfterBrowse,pickerClosed,duplicateHidden,removed,underlyingAssetRetained,orphanReferenceRemoved,restored,opened,highlighted,dropped,command,previewSource,selection:editor.selectedObjectId,history:editor.commandHistory.length};
  })()`)
  assert(bindingEvidence.afterBrowse === bindingEvidence.before && bindingEvidence.historyAfterBrowse === bindingEvidence.historyBefore && bindingEvidence.pickerClosed && bindingEvidence.opened.some((url) => url.includes(`asset=${editorEvidence.assetId}`)), `Choose from Media unexpectedly replaced the selected image: ${JSON.stringify(bindingEvidence)}`)
  assert(bindingEvidence.duplicateHidden && bindingEvidence.removed && bindingEvidence.underlyingAssetRetained && bindingEvidence.restored, `remove/reference safety workflow failed: ${JSON.stringify(bindingEvidence)}`)
