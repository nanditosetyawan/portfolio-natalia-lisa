...
try {
  ...
  await waitFor(`document.querySelectorAll('.draft-card').length===1`)
  const draftCardAccess = await evaluate(`(()=>{const card=document.querySelector('.draft-card');return {count:document.querySelectorAll('.draft-card').length,tabindex:card.getAttribute('tabindex')}})()`)
  assert(draftCardAccess.count === 1 && draftCardAccess.tabindex === '0', 'Draft Library item is not usable')
  await evaluate(`document.querySelector('.favorite-button').click()`)
  await waitFor(`document.querySelector('.favorite-button')?.textContent.trim()==='♥'`)
  ...
  await waitFor(`document.querySelectorAll('.draft-card').length===1`)
  await evaluate(`document.querySelector('.favorite-button').click()`)
  await waitFor(`Boolean(document.querySelector('.empty-state'))`)
