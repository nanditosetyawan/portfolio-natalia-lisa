...
try {
  ...
  const limits = await evaluate(`(async()=>{const defaults=await import('/src/data/default/site.ts');const snapshots=await import('/src/editor/editorSnapshot.ts');const instances=await import('/src/editor/editorInstances.ts');const make=(index,section)=>({instanceId:'limit-image-'+section.toLowerCase()+'-'+index,type:'image',sectionId:section.toLowerCase(),label:'Limit '+index,order:index,source:{kind:'media-assignment',assignmentEntityId:'limit-image-'+section.toLowerCase()+'-'+index},createdAt:new Date(0).toISOString()});const sectionSnapshot=snapshots.createEditorSnapshot(defaults.createDefaultSiteSnapshot());sectionSnapshot.instances=Array.from({length:instances.MAX_DYNAMIC_INSTANCES_PER_SECTION},(_,index)=>make(index,'Portfolio'));let sectionRejected=false;try{instances.assertCanInsertEditorInstance(sectionSnapshot,'Portfolio')}catch{sectionRejected=true}const totalSnapshot=snapshots.createEditorSnapshot(defaults.createDefaultSiteSnapshot());const sections=['Portfolio','About','Education','College','SHS','Experience','Certificate','Contact'];totalSnapshot.instances=Array.from({length:instances.MAX_DYNAMIC_INSTANCES_PER_SNAPSHOT},(_,index)=>make(index,sections[index%sections.length]));let totalRejected=false;try{instances.assertCanInsertEditorInstance(totalSnapshot,'Portfolio')}catch{totalRejected=true}return{sectionMaximum:instances.MAX_DYNAMIC_INSTANCES_PER_SECTION,totalMaximum:instances.MAX_DYNAMIC_INSTANCES_PER_SNAPSHOT,sectionRejected,totalRejected}})()`)
  assert(limits.sectionMaximum === 50 && limits.totalMaximum === 500 && limits.sectionRejected && limits.totalRejected, `dynamic instance safety limits were not enforced: ${JSON.stringify(limits)}`)

  await selectObject('portfolio-hero')
  await evaluate(`document.querySelector('.guest-home').dataset.phase038Root='stable'`)
  const baseline = await evaluate(`(async()=>{const pinia=document.querySelector('#app').__vue_app__.config.globalProperties.$pinia;const editor=pinia._s.get('editor');const library=pinia._s.get('media-library');await library.refresh();const selected=editor.objects.find(item=>item.id===editor.selectedObjectId);return{fixedAsset:editor.draftSnapshot.media.assignments.find(item=>item.entityId==='portfolio-profile-media')?.assetId,assets:library.assets.length,assetIds:library.assets.map(item=>item.id),instances:editor.draftSnapshot.instances.length,selected:{id:selected?.id,type:selected?.type,section:selected?.section},mediaInsert:document.querySelector('[data-property-key="media.upload"] input[type="file"]')?.disabled===false}})()`)
  assert(baseline.selected.id === 'portfolio-hero' && baseline.selected.type === 'Text' && baseline.selected.section === 'Portfolio' && baseline.mediaInsert, `Portfolio Text did not expose section-scoped insertion: ${JSON.stringify(baseline)}`)
  ...
  assert(afterGeometry.objects[insertedB.id].computed.width === '300px' && Math.abs(Number.parseFloat(afterGeometry.objects[insertedB.id].computed.height) - expectedDesktopHeight) < .02 && afterGeometry.objects[insertedB.id].computed.translate === '190px 35px' && afterGeometry.objects[insertedB.id].computed.rotate === '9deg', `Image B geometry/aspect lock did not render: ${JSON.stringify(afterGeometry.objects[insertedB.id])}`)
