...
try {
  ...
  try {
    ...
    await waitFor(`Boolean(document.querySelector('.edit-page'))&&!document.querySelector('.editor-recovery')&&Boolean(document.querySelector('[data-canvas-preset="mobile-390"]'))&&Boolean(document.querySelector('[data-editor-object-id="portfolio-hero"]'))`)
  } catch (error) {
  ...
  const canvasEvidence = await evaluate(`(async()=>{
    ...
    const widths={};for(const id of ['desktop-1440','desktop-1280','laptop-1024','tablet-768','mobile-390']){document.querySelector('[data-canvas-preset="'+id+'"]').click();await tick();widths[id]=document.querySelector('.preview-stage').style.width;}
    ...
    return {widths,sameRoot:root===document.querySelector('.guest-home')&&root.dataset.phase033Identity==='stable',checked,keyboardPreset,focusVisible:focus.outlineStyle!=='none'&&Number.parseFloat(focus.outlineWidth)>0,labels:buttons.map(button=>button.getAttribute('aria-label')),breakpoint:document.querySelector('.preview-stage').dataset.responsiveBreakpoint};
  })()`)
  assert(canvasEvidence.sameRoot && canvasEvidence.checked === 1 && canvasEvidence.keyboardPreset === 'desktop-1280' && canvasEvidence.focusVisible, `canvas identity or keyboard accessibility failed: ${JSON.stringify(canvasEvidence)}`)
  assert(JSON.stringify(canvasEvidence.widths) === JSON.stringify({ 'desktop-1440': '1440px', 'desktop-1280': '1280px', 'laptop-1024': '1024px', 'tablet-768': '768px', 'mobile-390': '390px' }), `canvas widths failed: ${JSON.stringify(canvasEvidence.widths)}`)
  assert(['Desktop 1440','Desktop 1280','Laptop 1024','Tablet 768','Mobile 390'].every((label) => canvasEvidence.labels.includes(label)), 'breakpoint labels are incomplete')
  ...
  const overrideEvidence = await evaluate(`(async()=>{
    ...
    await clickPreset('laptop-1024');await open('font');await setInput('[data-property-key="font.size"] input','44px');
    ...
    await clickPreset('mobile-390');await open('font');const inheritedValue=document.querySelector('[data-property-key="font.size"] input').value;const inheritedSource=document.querySelector('[data-property-key="font.size"]').closest('.property-field').dataset.responsiveSource;
    await setInput('[data-property-key="font.size"] input','36px');const mobileRecord=Object.entries(editor.draftSnapshot.typography).find(([key,value])=>key.startsWith('rwd-mobile-')&&value.fontSize==='36px');const overrideSource=document.querySelector('[data-property-key="font.size"]').closest('.property-field').dataset.responsiveSource;const computedMobile=getComputedStyle(smallest(id)).fontSize;const resetVisible=Boolean(document.querySelector('[aria-label="Reset Size override"]'));
    ...
    document.querySelector('[aria-label="Reset Size override"]').click();await waitFrames();const resetValue=document.querySelector('[data-property-key="font.size"] input').value;const resetSource=document.querySelector('[data-property-key="font.size"]').closest('.property-field').dataset.responsiveSource;
  ...
  assert(overrideEvidence.laptopRecord && overrideEvidence.mobileRecord && overrideEvidence.inheritedValue === '44px' && overrideEvidence.inheritedSource.includes('Laptop'), `sparse inheritance failed: ${JSON.stringify(overrideEvidence)}`)
  assert(overrideEvidence.overrideSource === 'Override' && overrideEvidence.computedMobile === '36px' && overrideEvidence.resetVisible && overrideEvidence.undoValue === '44px' && overrideEvidence.redoValue === '36px' && overrideEvidence.resetValue === '44px' && overrideEvidence.resetSource.includes('Laptop'), `override/undo/reset live Preview failed: ${JSON.stringify(overrideEvidence)}`)
