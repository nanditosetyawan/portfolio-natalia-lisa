...
const borderPreview: PropertyPreviewUpdater = {
  ...
  update: (context, value) => {
    ...
    const kind = findSnapshotObjectReference(snapshot, entityId)?.kind.toLocaleLowerCase()
...
const entries: PropertyRegistryEntry[] = [
  ...
  defineProperty({ propertyKey: 'media.aspectRatioLocked', category: 'media', categoryLabel: 'MEDIA', categoryOrder: 20, label: 'Lock proportions', control: 'checkbox', type: 'boolean', order: 52, commandType: 'SET_PROPERTY', capability: 'media', propertyPath: 'aspectRatioLocked', databaseMapping: snapshot('media.styles.{entityId}.aspectRatioLocked'), defaultValue: false, validation: validBoolean, dependency: supports('media-dimensions'), helperText: 'Keep width and height proportional.', styleKey: 'media.aspectRatioLocked' }),
